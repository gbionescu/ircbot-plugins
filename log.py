...
base_formats = {
    EventType.message: "[{server}:{channel}] {time} <{nick}> {content}",
    EventType.notice: "[{server}:{channel}] {time} -{nick}- {content}",
    EventType.action: "[{server}:{channel}] {time} * {nick} {content}",
    EventType.join: "[{server}:{channel}] {time} -!- {nick} [{user}@{host}] has joined",
    EventType.part: "[{server}:{channel}] {time} -!- {nick} [{user}@{host}] has left ({content})",
    EventType.kick: "[{server}:{channel}] {time} -!- {nick} has kicked {target} ({content})",
}
...
def format_event(event):
    ...
    args = {
        ...
        "user": event.user, "host": event.host, "time": time.strftime("%H:%M:%S", time.gmtime())
    }
